...
class SVMLinearClassifierWithProba(BaseEstimator, ClassifierMixin):
    ...
    def set_params(self, **params):
        ...
        # Обновляем SVM с новыми параметрами
        self.svm = LinearSVC(
            random_state=self.random_state,
            max_iter=self.max_iter,
            class_weight=self.class_weight,
    ...
    def fit(self, X, y):
        ...
        self.calibrator = CalibratedClassifierCV(self.svm, cv=min(3, len(np.unique(y))), method='sigmoid')
...
class BlendingClassifier(BaseEstimator, ClassifierMixin):
    """
    Кастомный Blending (блендинг) классификатор
    """

    def __init__(self, base_estimators, meta_estimator, test_size=0.3, random_state=42):
        """
        Args:
            base_estimators: список базовых классификаторов
            meta_estimator: мета-классификатор для блендинга
            test_size: доля данных для обучения мета-классификатора
    ...
    def fit(self, X, y):
        """Обучение модели блендинга"""
        # Разделяем данные на train и holdout
        X_train, X_holdout, y_train, y_holdout = train_test_split(
            X, y, test_size=self.test_size, random_state=self.random_state, stratify=y
        )

        # Обучаем базовые классификаторы на тренировочной части
        self.base_estimators_ = []
        ...
        # Получаем предсказания базовых классификаторов на holdout выборке
        base_predictions = []
        for name, estimator in self.base_estimators_:
            try:
                pred = estimator.predict_proba(X_holdout)[:, 1]  # вероятность класса 1
            except AttributeError:
                # Если predict_proba не доступен, используем predict
                pred = estimator.predict(X_holdout).astype(float)
        ...
        self.meta_estimator_.fit(meta_features, y_holdout)

        # Переобучаем базовые классификаторы на всех данных
        self.base_estimators_ = []
        for name, estimator in self.base_estimators:
            est = clone(estimator)
            est.fit(X, y)
    ...
    def predict_proba(self, X):
        ...
        for name, estimator in self.base_estimators_:
            try:
                pred = estimator.predict_proba(X)[:, 1]  # вероятность класса 1
            except AttributeError:
                # Если predict_proba не доступен, используем predict
        ...
        try:
            ...
            # Если мета-классификатор не имеет predict_proba
            pred = self.meta_estimator_.predict(meta_features).astype(float)
            # Создаем фиктивные вероятности
...
class MultiLabelBlendingClassifier:
    """
    Многометочный классификатор на основе Blending (блендинга)
    """

    def __init__(self, n_labels=14, base_estimators=None, meta_estimator='logistic',
                 test_size=0.3, n_jobs=-1, random_state=42):
        """
        Args:
    ...
    def _create_base_estimators(self):
        ...
        if self.base_estimators is None:
            ...
            return [
                ...
                ('svm', SVMLinearClassifierWithProba(
                    random_state=self.random_state,
                    max_iter=1000,
                    class_weight='balanced',
                    dual=False,
    ...
    def _create_meta_estimator(self):
        """Создает мета-классификатор"""
        if isinstance(self.meta_estimator, str):
            if self.meta_estimator == 'logistic':
                return LogisticRegression(
                    random_state=self.random_state,
                    max_iter=1000,
                    class_weight='balanced'
                )
            elif self.meta_estimator == 'svm':
                # Для мета-классификатора используем нашу обертку
                return SVMLinearClassifierWithProba(
                    random_state=self.random_state,
                    max_iter=1000,
                    class_weight='balanced',
                    dual=False,
            ...
            elif self.meta_estimator == 'tree':
                return DecisionTreeClassifier(
                    random_state=self.random_state,
                    max_depth=10,
                    class_weight='balanced'
                )
            elif self.meta_estimator == 'rf':
                ...
                raise ValueError(f"Неизвестный meta_estimator: {self.meta_estimator}")
        else:
            return self.meta_estimator

    def _create_blending_classifier(self):
        """Создает Blending классификатор"""
        base_estimators = self._create_base_estimators()
        meta_estimator = self._create_meta_estimator()

        # Сохраняем названия базовых классификаторов
        self.base_estimator_names = [name for name, _ in base_estimators]
    ...
    def train(self, train_data, val_data=None):
        ...
        """
        print("🎯 ОБУЧЕНИЕ МНОГОМЕТОЧНОГО BLENDING КЛАССИФИКАТОРА...")
        print("=" * 60)

        # Показываем архитектуру блендинга
        base_estimators = self._create_base_estimators()
        meta_estimator = self._create_meta_estimator()

        print("📊 Архитектура блендинга:")
        print(f"   Базовые классификаторы ({len(base_estimators)}):")
        for name, estimator in base_estimators:
            print(f"     - {name}: {estimator.__class__.__name__}")
        print(f"   Мета-классификатор: {meta_estimator.__class__.__name__}")
        print(f"   Размер holdout выборки: {self.test_size * 100:.0f}%")
        ...
        print("\n🥂 Обучение Blending для каждой метки...")
        ...
        for label_idx in range(self.n_labels):
            ...
            if len(unique_classes) < 2:
                ...
                # Если два класса, используем Blending
                print(f"   Метка {label_idx}: обучение Blending...")
                try:
                    clf = self._create_blending_classifier()
                    clf.fit(X_train_vec, y_single)
                except Exception as e:
                    print(f"   ⚠️ Ошибка при обучении Blending для метки {label_idx}: {e}")
                    print(f"   Используем DummyClassifier вместо Blending")
    ...
    def _predict_proba_from_estimators(self, X_vec):
        ...
        for idx, clf in enumerate(self.estimators_):
            if idx in self.single_class_labels:
                ...
                # Blending возвращает вероятности
                try:
                    prob = clf.predict_proba(X_vec)
                    probabilities.append(prob[:, 1])  # вероятность класса 1
                except:
                    # Если не удалось получить вероятности, используем 0.5
                    pred = clf.predict(X_vec)
                    prob = np.ones((len(pred), 2)) * 0.5
                    probabilities.append(prob[:, 1])
    ...
    def _analyze_model(self):
        """
        Анализ обученной модели Blending
        """
        print("\n🥂 АНАЛИЗ МОДЕЛИ BLENDING:")
        print("=" * 60)
        ...
        print(f"   Blending моделей: {blending_models}")
        print(f"   Dummy моделей: {dummy_models}")
        print(f"   Базовых классификаторов: {len(self.base_estimator_names)}")
        print(f"   Базовые классификаторы: {', '.join(self.base_estimator_names)}")
        print(f"   Размер holdout выборки: {self.test_size * 100:.0f}%")

        # Информация о мета-классификаторе
        for idx, clf in enumerate(self.estimators_):
            if idx not in self.single_class_labels and hasattr(clf, 'meta_estimator_'):
                print(f"\n   Мета-классификатор для метки {idx}:")
                print(f"      Тип: {clf.meta_estimator_.__class__.__name__}")
                if hasattr(clf.meta_estimator_, 'coef_'):
                    print(f"      Количество признаков: {clf.meta_estimator_.coef_.shape[1]}")
                break  # показываем только для первой метки
    ...
    def save_model(self, filename):
        ...
        joblib.dump({
            ...
            'n_jobs': self.n_jobs,
            'random_state': self.random_state,
            'base_estimator_names': self.base_estimator_names
        }, filename)
        print(f"💾 Модель Blending сохранена: {filename}")
    ...
    def load_model(self, filename):
        ...
        self.base_estimators = loaded.get('base_estimators', None)
        self.meta_estimator = loaded.get('meta_estimator', 'logistic')
        self.test_size = loaded.get('test_size', 0.3)
        self.n_jobs = loaded.get('n_jobs', -1)
        self.random_state = loaded.get('random_state', 42)
        self.base_estimator_names = loaded.get('base_estimator_names', [])
        self.is_trained = True
        print(f"📥 Модель Blending загружена: {filename}")
    ...
    def plot_meta_features_importance(self, max_labels=5):
        """
        Визуализация важности мета-признаков
        """
        valid_labels = [i for i in range(self.n_labels)
                        if i not in self.single_class_labels
                        and hasattr(self.estimators_[i].meta_estimator_, 'coef_')]

        if not valid_labels:
            print("Нет данных о важности мета-признаков")
            return
        ...
        for idx, label_idx in enumerate(valid_labels[:n_labels]):
            clf = self.estimators_[label_idx]
            coef = clf.meta_estimator_.coef_[0] if len(
                clf.meta_estimator_.coef_.shape) > 1 else clf.meta_estimator_.coef_

            # Получаем названия мета-признаков (базовые классификаторы)
            feature_names = self.base_estimator_names

            # Строим bar plot
            y_pos = np.arange(len(feature_names))
            colors = ['skyblue' if c > 0 else 'lightcoral' for c in coef]
            axes[idx].barh(y_pos, coef, align='center', color=colors)
            axes[idx].set_yticks(y_pos)
            axes[idx].set_yticklabels(feature_names)
            axes[idx].invert_yaxis()  # Самые важные сверху
            axes[idx].set_xlabel('Коэффициент важности')
            axes[idx].set_title(f'Метка {label_idx}')
            axes[idx].axvline(x=0, color='black', linestyle='-', linewidth=0.5)
            axes[idx].grid(True, alpha=0.3, axis='x')

        plt.suptitle('Важность базовых классификаторов в мета-модели', fontsize=14)
        plt.tight_layout()
        plt.show()
...
# Пример использования Blending
def main():
    """
    Пример использования многометочного Blending классификатора
    """
    ...
    print("\n" + "=" * 50)
    classifier = MultiLabelBlendingClassifier(
        n_labels=n_labels,
        test_size=0.3,
        n_jobs=-1
    )
    ...
    print("\n" + "=" * 50)
    print("🥂 ОБУЧЕНИЕ ФИНАЛЬНОЙ МОДЕЛИ BLENDING...")

    # Используем улучшенную конфигурацию
    classifier = MultiLabelBlendingClassifier(
        n_labels=n_labels,
        base_estimators=[
            ('lr', LogisticRegression(
    ...
    # 6. Сохраняем модель
    try:
        classifier.save_model("multilabel_blending.pkl")
    except Exception as e:
    ...
    try:
        ...
        classifier.plot_meta_features_importance(max_labels=3)
